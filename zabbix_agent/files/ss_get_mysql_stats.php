...
$timezone   = 'CET'; #null;    # If not set, uses the system default.  Example: "UTC"
$chk_options = array (
   ...
   'get_qrt' => false,   # was: true Get query response times from Percona Server or MariaDB?
);

$use_ss    = FALSE; # Whether to use the script server or not
$debug     = TRUE; # Define whether you want debugging behavior.
$debug_log = '/tmp/ss_get_mysql_debug.log'; # If $debug_log is a filename, it'll be used.
...
# ============================================================================
if ( file_exists('/usr/local/bin/' . basename(__FILE__) . '.cnf' ) ) {
   require('/usr/local/bin/' . basename(__FILE__) . '.cnf');
   debug('Found configuration file /usr/local/bin/' . basename(__FILE__) . '.cnf');
}
